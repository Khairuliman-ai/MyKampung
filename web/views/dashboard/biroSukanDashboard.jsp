...
<%
    ...
    Integer totalTempahanCount = (Integer) request.getAttribute("totalTempahanCount");
    if (totalTempahanCount == null) totalTempahanCount = 0;

    Long approvedTempahanCount = (Long) request.getAttribute("approvedTempahanCount");
    if (approvedTempahanCount == null) approvedTempahanCount = 0L;
%>
...
<div class="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth h-full bg-[#F8FAFC] custom-scrollbar">
    <div class="max-w-7xl mx-auto">
        ...
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 border border-blue-100 text-blue-600 text-[10px] font-extrabold uppercase tracking-widest">
                    <i class="fas fa-calendar-alt"></i> Portal Sukan
                </span>
                <h1 class="text-3xl font-black text-slate-800 tracking-tight mt-2">Papan Pemuka Sukan & Riadah</h1>
                <p class="text-slate-500 text-sm mt-0.5">Urus kelulusan slot tempahan kemudahan awam kampung seperti dewan komuniti dan gelanggang sukan.</p>
            </div>
            ...
            <div class="relative w-full md:w-80 group">
                ...
                <input type="text" 
                       class="w-full pl-11 pr-4 py-3 bg-white rounded-2xl border border-slate-200 focus:ring-4 focus:ring-blue-100 focus:border-blue-400 shadow-sm text-sm placeholder-gray-400 transition-all outline-none" 
                       placeholder="Cari tempahan fasiliti...">
            </div>
        </header>

        <!-- Premium Hero Section -->
        <div class="relative bg-gradient-to-r from-[#1E3A8A] to-[#0284C7] rounded-[2.5rem] p-8 md:p-10 text-white mb-8 shadow-xl overflow-hidden group">
            <div class="relative z-10 max-w-xl">
                <span class="bg-white/20 text-[10px] font-extrabold px-3 py-1 rounded-full backdrop-blur-md text-sky-300 uppercase tracking-widest border border-white/10">BIRO SUKAN & RIADAH</span>
                <h1 class="text-3xl md:text-4xl font-black mt-4 mb-2 leading-tight">Selamat Datang, <%= user.getNama_penuh() %>!</h1>
                <p class="text-sky-100 mb-6 text-sm leading-relaxed opacity-95">
                    Permudah aktiviti riadah penduduk dengan sistem tempahan slot yang pantas dan telus. Pastikan tiada pertindihan jadual sebelum meluluskan tempahan.
                </p>
                <div class="flex flex-wrap gap-3">
                    <a href="<%= request.getContextPath() %>/tempahan/urus" class="bg-white text-blue-900 px-6 py-3 rounded-xl font-bold text-xs hover:bg-slate-100 transition shadow-md flex items-center gap-2">
                        <i class="fas fa-tasks"></i> Urus Semua Tempahan
                    </a>
                </div>
            </div>
            ...
            <div class="absolute bottom-0 right-20 w-48 h-48 bg-blue-950 opacity-20 rounded-full blur-2xl"></div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="glass-card p-6 rounded-3xl flex items-center gap-4 relative overflow-hidden">
                ...
                <div>
                    <span class="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider block">Jumlah Rekod Tempahan</span>
                    <span class="text-2xl font-black text-slate-800 tracking-tight block mt-0.5"><%= totalTempahanCount %> <span class="text-xs font-normal text-slate-400">slot</span></span>
                </div>
            </div>
            ...
            <div class="glass-card p-6 rounded-3xl flex items-center gap-4 relative overflow-hidden">
                ...
                <div>
                    ...
                    <span class="text-2xl font-black text-slate-800 tracking-tight block mt-0.5"><%= pendingTempahanCount %> <span class="text-xs font-normal text-slate-400">kes</span></span>
                </div>
            </div>
            ...
            <div class="glass-card p-6 rounded-3xl flex items-center gap-4 relative overflow-hidden">
                ...
                <div>
                    <span class="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider block">Slot Diluluskan</span>
                    <span class="text-2xl font-black text-slate-800 tracking-tight block mt-0.5"><%= approvedTempahanCount %> <span class="text-xs font-normal text-slate-400">lulus</span></span>
                </div>
            </div>
        </div>

        <!-- Table: Pending Booking Applications -->
        <div class="glass-card rounded-[2.5rem] p-6 mb-8 overflow-hidden flex flex-col">
            <div class="flex justify-between items-center mb-6 pb-4 border-b border-slate-100/50">
                <div>
                    <h3 class="font-black text-lg text-slate-800">Permohonan Tempahan Baru Menunggu Sokongan</h3>
                    <p class="text-xs text-slate-400">Semak slot tarikh dan masa sebelum memberikan kelulusan rasmi.</p>
                </div>
                <span class="px-2.5 py-1 rounded-full bg-orange-100 text-orange-600 text-[10px] font-extrabold uppercase"><%= pendingTempahanCount %> Baru</span>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-slate-100 text-slate-400 text-[10px] font-extrabold uppercase tracking-wider">
                            ...
                            <th class="pb-3">Fasiliti</th>
                            <th class="pb-3">Tarikh & Masa Slot</th>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-9 h-9 rounded-xl bg-blue-50 border border-blue-100 text-blue-600 flex items-center justify-center text-xs font-black">
                                            <%= tf.getNama_pengguna() != null ? tf.getNama_pengguna().substring(0,1).toUpperCase() : "T" %>
                                        </div>
                                        <div>
                                            <div class="text-sm font-bold text-slate-800"><%= tf.getNama_pengguna() %></div>
                                            <div class="text-[10px] text-slate-400 font-bold mt-0.5"><i class="far fa-user"></i> Penduduk</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-4 text-xs font-bold text-slate-500">
                                    <div><i class="far fa-calendar-alt"></i> <%= tf.getTarikh_tempah() %></div>
                                    <div class="text-[10px] text-slate-400 font-bold mt-0.5"><i class="far fa-clock"></i> <%= tf.getMasa_mula() %> - <%= tf.getMasa_tamat() %></div>
                                </td>
                                <td class="py-4 text-center">
                                    <a href="<%= request.getContextPath() %>/tempahan/urus" class="px-4 py-2 bg-slate-50 hover:bg-slate-800 hover:text-white text-slate-700 border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all">
                                        Sahkan
                                    </a>
                                </td>
                            </tr>
...
<aside class="hidden xl:flex w-full xl:w-80 bg-white border-t xl:border-t-0 xl:border-l border-slate-100 flex-col p-8 flex-shrink-0 h-full overflow-y-auto custom-scrollbar shrink-0">
    ...
    <div class="text-center mb-8 pb-8 border-b border-slate-100">
        <div class="relative w-24 h-24 mx-auto mb-4">
            ...
            <% } else { %>
                <img src="https://ui-avatars.com/api/?name=<%= user.getNama_penuh() %>&background=1E3A8A&color=fff&size=128" 
                     class="w-full h-full rounded-[2rem] object-cover border-4 border-white shadow-lg relative z-10">
            <% } %>
        ...
        <p class="text-[10px] font-extrabold text-blue-600 bg-blue-50 border border-blue-100 px-3.5 py-1 rounded-full inline-block mt-2 uppercase tracking-wider">
            <%= (user.getNama_jawatan() != null) ? user.getNama_jawatan() : "Biro Sukan" %>
        </p>
    </div>
    ...
    <div class="mt-auto">
        <h4 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">Nota Pengurusan Sukan</h4>
        <div class="bg-gradient-to-br from-[#1E3A8A] to-[#0369A1] rounded-3xl p-5 border border-white/5 relative overflow-hidden group">
            <i class="fas fa-scroll absolute -right-2 -bottom-2 text-blue-950 text-6xl opacity-40"></i>
            <p class="text-[11px] text-blue-200 leading-relaxed relative z-10 font-medium">
                Peringatan: Luluskan tempahan berdasarkan kekosongan slot rasmi yang disahkan. Pastikan penduduk mematuhi Garis Panduan Kebersihan dewan dan kemudahan selepas tamat acara.
            </p>
        </div>
